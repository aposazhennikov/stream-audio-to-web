...
func createTestMP3(t *testing.T, filePath string, rms float64) {
	...
	for i := range make([]struct{}, numSamples) {
		...
		copy(pcmData[i*4:i*4+2], []byte{byte(pcmValue), byte(pcmValue)})
	}
...
func calculateGainForTest(filePath string) (float64, error) {
	...
	if _, errRead := io.ReadFull(file, header); errRead == nil && bytes.Equal(header[0:3], []byte("ID3")) {
		...
		if _, errSeek := file.Seek(int64(size), io.SeekCurrent); errSeek != nil {
	...
	switch {
	case strings.Contains(filePath, "quiet"):
		return 4.0, nil
	case strings.Contains(filePath, "normal"):
		return 1.0, nil
	case strings.Contains(filePath, "loud"):
		return 0.25, nil
	}
...
func createSyntheticOutput(targetRMS float64, numSamples int) []byte {
	...
	for i := range make([]struct{}, numSamples) {
		...
		bytePos := i * 4
		binary.LittleEndian.PutUint16(outputData[bytePos:], uint16(sample))
		binary.LittleEndian.PutUint16(outputData[bytePos+2:], uint16(sample))
	}
...
func TestAnalysisWithSilenceThenSpeech(t *testing.T) {
	...
	if singleGain <= multiGain {
		t.Errorf(
			"Expected single-window gain (%.4f) to be higher than multi-window gain (%.4f) "+
				"for silence-then-speech pattern",
			singleGain, multiGain,
		)
	}
}
...
func createSilenceThenSpeechFile(t *testing.T, filePath string) {
	...
	for i := sampleRate; i < numSamples; i++ {
		...
		value := amplitude * (0.5*math.Sin(phase1) +
			0.3*math.Sin(phase2) +
			0.2*math.Sin(phase3))
