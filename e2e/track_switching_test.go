...
func testNextTrackAPI(t *testing.T, client *http.Client, baseURL, routeName string) {
	...
	// Правильно извлекаем строковое значение track.
	initialTrack, ok := initialTrackInfo["track"]
	if !ok {
		t.Fatalf("Track information missing 'track' field")
	}

	initialTrackName, ok := initialTrack.(string)
	if !ok {
		t.Fatalf("Track field is not a string as expected")
	}
	...
	// Правильно извлекаем строковое значение track.
	newTrack, ok := newTrackInfo["track"]
	if !ok {
		t.Fatalf("New track information missing 'track' field")
	}

	newTrackName, ok := newTrack.(string)
	if !ok {
		t.Fatalf("New track field is not a string as expected")
	}
