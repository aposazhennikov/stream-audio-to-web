...
func TestTrackControlEndpoints(t *testing.T) {
	...
	if jsonUnmarshalErr := json.Unmarshal(bodyBytes, &currentTrackInfo); jsonUnmarshalErr != nil {
		t.Fatalf("Failed to decode current track info: %v, body: %s", jsonUnmarshalErr, responseBody)
	}

	currentTrack, trackNameOk := currentTrackInfo["track"].(string)
	if !trackNameOk {
	...
	newPlayingResp, getNewNowPlayingErr := client.Get(fmt.Sprintf("%s/now-playing?route=/%s", baseURL, testRoute))
	if getNewNowPlayingErr != nil {
		t.Fatalf("Failed to get new current track: %v", getNewNowPlayingErr)
	}
	...
	// Read response body for analysis
	newBodyBytes, readNewNowPlayingBodyErr := io.ReadAll(newPlayingResp.Body)
	newPlayingResp.Body.Close()
	if readNewNowPlayingBodyErr != nil {
		t.Fatalf("Failed to read now-playing response body: %v", readNewNowPlayingBodyErr)
	}
	...
	if err2 := json.Unmarshal(newBodyBytes, &newTrackInfo); err2 != nil {
		t.Fatalf("Failed to decode new track info: %v, body: %s", err2, newResponseBody)
	}

	newTrack, newTrackNameOk := newTrackInfo["track"].(string)
	if !newTrackNameOk {
