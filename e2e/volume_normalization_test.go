...
func TestVolumeNormalization(t *testing.T) {
	...
	routes = getSortedKeys(routeLUFS)
...
func TestFirstSoundLatency(t *testing.T) {
	...
	require.NoError(t, err, "Failed to make GET request to %s", testURL)
	...
	for bytesRead < 1024 {
		n, err := resp.Body.Read(buffer[bytesRead:])
...
func collectAudioSamples(t *testing.T, url string, minBytes int, maxTime time.Duration) [][]byte {
	...
	require.NoError(t, err, "Failed to make GET request to %s", url)
	defer resp.Body.Close()
...
func convertToPCM(samples [][]byte) []int16 {
	...
	pcmIndex := 0
	for _, sample := range samples {
		for i := 0; i < len(sample)-1; i += 2 {
			if pcmIndex < len(pcmSamples) {
				pcmSamples[pcmIndex] = int16(binary.LittleEndian.Uint16(sample[i:]))
				pcmIndex++
			}
		}
	}
...
// Helper function to get sorted map keys
...
func TestVolumeJumpsOnTrackChange(t *testing.T) {
	...
	// Create an HTTP client with cookie for authentication
	client := &http.Client{}
...
func switchToNextTrack(t *testing.T, serverURL, route, password string) {
	...
	// Create request
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/next-track%s", serverURL, route), nil)
...
func TestMultiWindowAnalysisLatency(t *testing.T) {
	...
	startTime = time.Now()
	singleGain, err := audio.CalculateGain(lectureFile, routeName + "_single")
...
func TestVolumeNormalizationWithPodcast(t *testing.T) {
	...
	// Open the files
	silenceFile, err := os.Open(silenceFirstFile)
	require.NoError(t, err, "Failed to open silence_first.mp3")
	defer silenceFile.Close()
	
	loudFile, err := os.Open(loudFirstFile)
	require.NoError(t, err, "Failed to open loud_first.mp3")
