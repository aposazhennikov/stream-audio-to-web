...
func TestPlaylistUpdate(t *testing.T) {
	...
	slog.Info("Getting initial track list for route", slog.String("route", route))
	...
	slog.Info("Copying test file", slog.String("src", testAudioFile), slog.String("dst", destFilePath))
	...
	defer func() {
		slog.Info("Removing test file", slog.String("file", destFilePath))
	...
	slog.Info("Waiting for playlist update...")
	...
	if !foundNewFile {
		...
		slog.Info("New file successfully added to playlist", slog.String("file", destFileName))
	}
}
